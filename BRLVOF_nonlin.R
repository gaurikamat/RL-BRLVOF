...
for(s in 1:nsim){
  ...
  Y_1_m=rnorm(n_m,10+cond_assoc*(a_m1*X_m[,1]+a_m2*X_m[,2]+a_m3*X_m[,3]+a_m4*X_m[,4])+0.1*(X_m[,1]^2+X_m[,2]^2+X_m[,3]^2+X_m[,4]^2), error_sd)
 ...
 #Unique match IDs
  ...
  Y_1_A=rnorm(N_A-n_m,5+cond_assoc*(a_u1*X_A[,1]+a_u2*X_A[,2]+a_u3*X_A[,3]+a_u4*X_A[,4])+0.03*(X_A[,1]^2+X_A[,2]^2+X_A[,3]^2+X_A[,4]^2),error_sd)
  ...
  Y_1_B=rnorm(N_B-n_m,5+cond_assoc*(a_u1*X_B[,1]+a_u2*X_B[,2]+a_u3*X_B[,3]+a_u4*X_B[,4])+0.03*(X_B[,1]^2+X_B[,2]^2+X_B[,3]^2+X_B[,4]^2),error_sd)
