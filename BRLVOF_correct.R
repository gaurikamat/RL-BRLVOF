...
for(s in 1:nsim){
  ...
  Y_1_m=rnorm(n_m,10+cond_assoc*(a_m1*X_m[,1]+a_m2*X_m[,2]), error_sd)
  ...
  Y_1_A=rnorm(N_A-n_m,5+cond_assoc*(a_u1*X_A[,1]+a_u2*X_A[,2]),error_sd)
  ...
  Y_1_B=rnorm(N_B-n_m,5+cond_assoc*(a_u1*X_B[,1]+a_u2*X_B[,2]),error_sd)
